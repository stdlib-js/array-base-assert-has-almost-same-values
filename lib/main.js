...
var isAlmostSameValueF16 = require( '@stdlib/number-float16-base-assert-is-almost-same-value' );
...
function hasAlmostSameValues( x, y, maxULP ) {
	...
	if ( xo.accessorProtocol || yo.accessorProtocol ) {
		...
		if ( FLG === 0 ) {
			...
			return internal( xr, yr, maxULP, isAlmostSameValueF16 );
		}
		return accessors( xo, yo, maxULP, isAlmostSameValue ); // general accessors should always use general comparison
	}
	// Determine whether we should use double-precision, single-precision, or half-precision comparison...
	if ( x.BYTES_PER_ELEMENT === 2 && y.BYTES_PER_ELEMENT === 2 ) {
		return internal( x, y, maxULP, isAlmostSameValueF16 );
	}
